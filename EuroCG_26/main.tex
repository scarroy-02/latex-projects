...
The \textbf{evolute} of $\mathcal{M}$ is the curve $E(s)$ defined by the centers of osculating circles {of the plane curve} $\mathcal{M}$:
\begin{equation}
...
Geometrically, the evolute can also be characterized as the envelope of the family of lines normal to $\mathcal{M}$. Additionally, it can be viewed as the locus of the centers of circles whose 2-jet coincide with the curve at the point of contact of the curve and the circle.
...
The vineyard $\mathcal{V}$ is constructed by stack-wise concatenation of persistence diagrams generated from a parameterized family of distance functions. Methods have been proposed by e.g. Dey et al. \cite{tamalrevisit} to compute vineyards effectively, but an elementary approach suffices here. %we go for a more primitive approach. 
In our tool, we parameterize the filtration by moving the source point $\gamma(t)$ along a circular path of user-defined radius and center.
